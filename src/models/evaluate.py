...
from ..utils import save_metrics, setup_logging
...
class ModelEvaluator:
    ...
    def compute_metrics(
        self, y_true: pd.Series, y_pred: np.ndarray, y_prob: Optional[np.ndarray] = None
    ) -> Dict[str, float]:
        """
        ...
        try:
            ...
            if y_prob is not None:
                try:
                    ...
                    metrics["roc_auc"] = 0.0
                    self.logger.warning(
                        "Could not compute ROC AUC (only one class present)"
                    )
                ...
                try:
                    ...
                    metrics["pr_auc"] = 0.0
                    self.logger.warning(
                        "Could not compute PR AUC (only one class present)"
                    )
    ...
    def optimize_threshold(
        self, y_true: pd.Series, y_prob: np.ndarray, metric: str = "f1"
    ) -> Tuple[float, float]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Optimal threshold: {optimal_threshold:.3f} (score: {best_score:.4f})"
            )
        ...
        try:
            ...
            if hasattr(model, "predict_proba"):
                try:
                    ...
                    self.logger.warning(
                        f"Could not get probabilities for {model_name}: {str(e)}"
                    )

            # Optimize threshold for fraud detection
            if y_prob is not None:
                optimal_threshold, best_f1 = self.optimize_threshold(
                    y_test, y_prob, "f1"
                )
    ...
    def evaluate_all_models(
        ...
        try:
            ...
            self.logger.info(
                f"Evaluation completed for {len(self.evaluation_results)} models"
            )
    ...
    def plot_metrics_comparison(
        ...
        try:
            ...
            for metric in metrics_to_plot:
                if any(
                    metric in model_metrics for model_metrics in metrics_dict.values()
                ):
            ...
            self.logger.info(
                f"Plotted metrics comparison for {len(available_metrics)} metrics"
            )
    ...
    def generate_evaluation_report(
        ...
        try:
            ...
            report = {"summary": {}, "detailed_results": {}, "recommendations": []}
            ...
            if self.evaluation_results:
                ...
                for model_name, result in self.evaluation_results.items():
                    ...
                    if recall < 0.7:
                        recommendations.append(
                            f"{model_name}: Low recall ({recall:.3f}). Consider adjusting threshold or using different resampling."
                        )
                ...
                for model_name, result in self.evaluation_results.items():
                    ...
                    if precision < 0.7:
                        recommendations.append(
                            f"{model_name}: Low precision ({precision:.3f}). Consider feature engineering or different model."
                        )
                ...
                for model_name, result in self.evaluation_results.items():
                    ...
                    if accuracy > 0.95:
                        recommendations.append(
                            f"{model_name}: Very high accuracy ({accuracy:.3f}). Potential overfitting - check cross-validation."
                        )
