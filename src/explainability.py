...
class ModelExplainer:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not SHAP_AVAILABLE and self.config.get('use_shap', True):
            self.logger.warning("SHAP not available. SHAP-based explanations will be disabled.")
    ...
    def fit_shap_explainer(
        ...
        try:
            if not self.config.get('use_shap', True) or not SHAP_AVAILABLE:
                self.logger.info("SHAP explainer skipped (not configured or not available)")
                return
            ...
            self.logger.info(f"SHAP explainer fitted successfully for {model_type} model")
    ...
    def get_feature_importance(
        ...
        try:
            ...
            if method == "shap" and self.shap_explainer is not None:
                ...
                if hasattr(model, 'feature_importances_'):
                    ...
                    if shap_values is not None:
                        ...
                        raise ValueError("Could not compute SHAP-based feature importance")
            ...
            feature_names = feature_names or self.feature_names or [f"feature_{i}" for i in range(len(importance_scores))]
            importance_dict = dict(zip(feature_names, importance_scores))
            
            # Sort by importance
            importance_dict = dict(sorted(importance_dict.items(), key=lambda x: x[1], reverse=True))
            
            self.logger.info(f"Computed feature importance for {len(importance_dict)} features")
    ...
    def plot_feature_importance(
        ...
        try:
            ...
            self.logger.info(f"Plotted feature importance for top {len(features)} features")
    ...
    def plot_prediction_explanation(
        ...
        try:
            ...
            contributions = [item["contribution"] for item in explanation["top_features"]]
                ...
                if shap_values is not None:
                    ...
                    for idx in sample_indices:
                        ...
                        if hasattr(model, 'predict'):
                            ...
                            sample_explanation = self.explain_prediction(sample_X, sample_pred, shap_values[idx:idx+1])
