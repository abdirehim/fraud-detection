...
class TestDataPreprocessor:
    ...
    def test_clean_data(self):
        ...
        df_with_issues = pd.concat(
            [df_with_issues, df_with_issues.iloc[:5]]
        )  # Add duplicates
    ...
    def test_encode_categorical_features(self):
        ...
        categorical_cols = encoded_df.select_dtypes(
            include=["object", "category"]
        ).columns
        assert "test_categorical" not in categorical_cols or encoded_df[
            "test_categorical"
    ...
    def test_fit_transform_pipeline(self):
        ...
        assert len(transformed_df.columns) >= len(
            df.columns
    ...
    def test_config_override(self):
        ...
        custom_config = {"scaling_method": "robust", "target_column": "fraud"}
    ...
    def test_error_handling_invalid_resampling_method(self):
        ...
        with pytest.raises(ValueError, match="Unsupported resampling method"):
            preprocessor.handle_imbalanced_data(
                df.drop(columns=[target_col]), df[target_col], method="invalid_method"
            )
