...
from src.utils import create_experiment_dir, setup_logging
...
def main():
    ...
    parser.add_argument(
        "--target-col", type=str, default="fraud", help="Target column name"
    )
    ...
    try:
        ...
        if args.target_col not in df.columns:
            if "class" in df.columns:
                ...
                raise ValueError(
                    f"Target column '{args.target_col}' not found in dataset"
                )
        ...
        logger.info(
            f"Processed data: {df_processed.shape[0]} rows, {df_processed.shape[1]} columns"
        )
        ...
        logger.info(
            f"Converted test target to binary: {y_test_binary.value_counts().to_dict()}"
        )

        evaluator = ModelEvaluator()
        evaluation_results = evaluator.evaluate_all_models(
            models, X_test, y_test_binary
        )
